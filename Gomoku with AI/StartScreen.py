...
class StartScreen:
    ...
    def mousePressedWrapper(self, event):
        ...
        if self.chooseColor:
            ...
            if self.player1 == 'computer' or self.player2 == 'computer':
                self.isChooseDifficult = True
                self.chooseColor = False
            else:
                self.isChooseDifficult = False
                self.isOver = True
            return
        if self.isChooseDifficult:
            self.mode = 1 if event.x < self.width / 2 else 2
    ...
    def drawChoicePage(self):
        ...
        # self.canvas.create_text(self.width/2,30,text = "click to choos",anchor = E,fill = "black",
        # font = ("Fixedsys", 25)) self.canvas.create_text(self.width/2,30,text = "e your opponent",anchor = W,
        # fill = "white",font = ("Fixedsys", 25))
        self.canvas.create_text(50, self.height / 2, text="Human", fill="black", anchor=W, font=("Fixedsys", 40))
        self.canvas.create_text(self.width / 4, self.height / 2 + 35, text=""" Yes\n I have a friend play with me""",
                                fill="black", font=("Fixedsys", 8))
        self.canvas.create_text(self.width / 4 * 3, self.height / 2, text="Computer", fill="white",
                                font=("Fixedsys", 40))
        self.canvas.create_text(self.width / 4 * 3, self.height / 2 + 35,
    ...
    def drawStartScreening(self):
        ...
        self.canvas.create_text(self.width / 2, self.height / 3, text='GOMOKU', font=('Courier', self.fontSize, 'bold'),
                                fill="white")
        fill = "white" if self.textcolor else "black"
        self.canvas.create_text(self.width / 2, self.height * 2 / 3, text='Press enter to start', font=('Fixedsys', 15),
                                fill=fill)
    ...
    def drawChoiceColorPage(self):
        ...
        self.canvas.create_text(self.width / 4, self.height / 2 + 35, text=""" Oh!\n I'm good at defense      """,
                                fill="white", font=("Fixedsys", 8))
        self.canvas.create_text(self.width / 4 * 3, self.height / 2, text="Black    ", fill="Black",
                                font=("Fixedsys", 40))
        self.canvas.create_text(self.width / 4 * 3, self.height / 2 + 35,
    ...
    def drawDiffcultPage(self):
        ...
        self.canvas.create_text(self.width / 4, self.height / 2 + 35,
                                text=""" Easy, and \n he doesn't even think in the game""", fill="black",
                                font=("Fixedsys", 8))
        self.canvas.create_text(self.width / 4 * 3, self.height / 2, text="Hard", fill="white", font=("Fixedsys", 40))
        self.canvas.create_text(self.width / 4 * 3, self.height / 2 + 35,
                                text=""" Hard, but \n he needs more time to make decision""", fill="white",
                                font=("Fixedsys", 8))
    # self.canvas.create_text(self.width/2,60,text = "check the ru",anchor = E,fill = "black",font = ("Fixedsys", 25))
    # self.canvas.create_text(self.width/2,60,text = "le online :)",anchor = W,fill = "white",font = ("Fixedsys", 25))
