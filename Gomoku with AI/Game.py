...
class Game(object):
    ...
    def takeTurn(self, event):
        if self.currentPlayer[0] == "human" and self.inComputerTurn is False:
    ...
    def computerWrapper(self):
        ...
        maxValue = self.scores[0][0]
        for tup in self.scores:
            if (self.currentPlayer[1] == 'black' and tup[0] > maxValue
                    or self.currentPlayer[1] != 'black' and tup[0] < maxValue):
                maxValue = tup[0]
                res = [(tup[1], tup[2])]
            elif (self.currentPlayer[1] == 'black' and tup[0] == maxValue
                  or self.currentPlayer[1] != 'black' and tup[0] == maxValue):
                res += [(tup[1], tup[2])]
        row, col = random.choice(res)
        self.inputInfo(row, col)
