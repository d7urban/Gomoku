...
def mousePressed(event, data):
    ...
    elif data.isStart and data.g.board.isWin is False and data.g.inComputerTurn is False and not data.g.inSelectHuman:
...
def timerFired(data):
    ...
    if not data.s.isOver and data.anytime % 600 == 0:
        data.s.textcolor = not data.s.textcolor
    data.anytime += 10
    if data.s.isOver and data.isStart is False:
        data.g = Game(data.s.info, data)

        data.isStart = True
    if data.isStart is True and data.g.currentPlayer[0] == 'computer' and not data.g.board.isWin:
        data.g.inComputerTurn = True
        # redrawAll(data.canvas, data)
        data.g.computerTurn()

    if data.isStart is True and data.g.board.isWin:
...
def redrawAll(canvas, data):
    ...
    if not data.s.isOver and data.s.chooseColor:
        ...
        if data.g.board.isWin:
            data.drawWin.drawWin(canvas, data.g.notCurrentPlayer[1].upper(), data.g.notCurrentPlayer[0],
                                 data.g.currentPlayer[0])
...
def run(width=300, height=300):
    def redrawAllWrapper(canvas, local_data):
        canvas.delete(ALL)
        redrawAll(canvas, local_data)
        canvas.update()

    def mousePressedWrapper(event, canvas, local_data):
        mousePressed(event, local_data)
        redrawAllWrapper(canvas, local_data)

    def keyPressedWrapper(event, canvas, local_data):
        keyPressed(event, local_data)
        redrawAllWrapper(canvas, local_data)

    def timerFiredWrapper(canvas, local_data):
        timerFired(local_data)
        redrawAllWrapper(canvas, local_data)
        canvas.after(local_data.timerDelay, timerFiredWrapper, canvas, local_data)
