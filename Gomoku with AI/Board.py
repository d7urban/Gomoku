def checkFromCellInDirection(board, startRow, startCol, index):
...
def checkFromCell(board, startRow, startCol):
    result = False
    return any(
        checkFromCellInDirection(board, startRow, startCol, index)
...
class Board(object):
    ...
    def checkBoard(self):
        ...
        for row in range(len(self.board)):
            for col in range(len(self.board[row])):
                if checkFromCell(self.board, row, col) and self.board[row][col] != -1:
                    self.isWin = True
                    return True
        return False
        ...
        for row in range(self.row):
            for col in range(self.col):
                if self.board[row][col] != -1:
                    ...
                    fill = "black" if self.board[row][col] == "white" else "white"
    ...
    def getScore(self):
        ...
        self.dectCol()
        self.dectDia()
        return self.calculate(self.blackCount, self.whiteCount)

    def calculate(self, bc, wc):
        res = 0
    ...
    def get_backward_diagonals(grid):
        ...
        return [[c for c in r if c is not None] for r in Board.get_cols(grid)]
    ...
    def transfer(self, line, flag):
        global strLine
        if flag == 'black':
            strLine = self.transfer2('w', line)
        elif flag == 'white':
            strLine = self.transfer2('b', line)
        return strLine

    # TODO Rename this here and in `transfer`
    def transfer2(self, arg0, line):
        result = arg0
        for s in line:
            if s == 'black':
                result += 'b'
    ...
    def detect(self, strLine, flag):
        while strLine.find(flag) != -1:
            ...
            block = 1 if strLine[start - 1] not in ['0', flag] else 0
    ...
    def modify(self, flag, run, block):
        if flag == 'w':
            if run == 5:
                ...
                self.whiteCount[(run, block)] += 1
        elif flag == 'b':
            if run == 5:
                self.blackCount[5] += 1
            elif run < 6:
                self.blackCount[(run, block)] += 1
